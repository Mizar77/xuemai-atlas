...
import {
  candidatePriorityP0MainlandSecondPassBatch1DispositionOverrides2026 as overrides,
  candidatePriorityP0MainlandSecondPassBatch1People2026 as people,
  candidatePriorityP0MainlandSecondPassBatch1Relationships2026 as relationships,
  candidatePriorityP0MainlandSecondPassBatch1RosterPromotions2026 as promotions,
  candidatePriorityP0MainlandSecondPassBatch1SupportingPeople2026 as supportingPeople,
} from "../app/candidate-priority-p0-mainland-second-pass-batch-1-2026.ts";

const sourceAuditPath = "data/candidate-priority-p0-mainland-second-pass-source-audit-2026-09-03.json";
...
const existingData = readdirSync("app").filter((file) => file.endsWith(".ts") && file !== "candidate-priority-p0-mainland-second-pass-batch-1-2026.ts").map((file) => readFileSync(`app/${file}`, "utf8")).join("\n");
for (const match of existingData.matchAll(/id:\s*["']([^"']+)["']/g)) allIds.add(match[1]);

if (sourceAudit.candidateCount !== 854 || sourceAudit.reviewedCount !== 854) errors.push("second-pass source audit is not a complete 854/854 ledger");
...
for (const override of overrides) {
  if (!currentKeys.has(override.canonicalKey)) errors.push(`${override.canonicalKey}: override not present in 854-row second-pass ledger`);
}
...
writeFileSync("data/candidate-priority-p0-mainland-second-pass-batch-1-audit-2026-09-03.json", `${JSON.stringify(result, null, 2)}\n`);
if (errors.length) {
