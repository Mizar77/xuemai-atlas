...
const bundle = "/private/tmp/candidate-p0-europe-full-batch-1-2026.mjs";
execFileSync(path.join(root, "node_modules/.bin/esbuild"), [path.join(root, "app/candidate-priority-p0-europe-full-batch-1-2026.ts"), "--bundle", "--platform=node", "--format=esm", `--outfile=${bundle}`], { stdio: "ignore" });
...
if (people.length !== 2 || rosterPromotions.length !== 2) errors.push("expected 2 people and promotions");
if (ids.size !== people.length) errors.push("duplicate person id");
for (const person of people) {
  if (!person.primary || (person.sources ?? []).length < 2 || (person.facts ?? []).length < 3 || person.facts.length > 5) errors.push(`${person.name}: strict source/fact gate failed`);
  ...
  if (!groupMembers.some((row) => row.teacherId === person.id && row.source?.url && row.source?.supports)) errors.push(`${person.name}: first-party network gate failed`);
}
...
console.log(JSON.stringify({ ok: true, primaryPeople: people.length, relationships: relationships.length, placements: placements.length, groupMembers: groupMembers.length, rosterPromotions: rosterPromotions.length }, null, 2));
