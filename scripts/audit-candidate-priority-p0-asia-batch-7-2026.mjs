...
const modulePath = path.join(root, "app/candidate-priority-p0-asia-batch-7-2026.ts");
const bundlePath = "/private/tmp/candidate-p0-asia-batch-7-2026.mjs";
...
const { people, relationships, placements, groupMembers, rosterPromotions, duplicateRosterPromotions } = await import(`file://${bundlePath}?t=${Date.now()}`);
const errors = [];
const ids = new Set(people.map((person) => person.id));
if (people.length !== 10 || ids.size !== 10) errors.push(`expected 10 unique people, got ${people.length}/${ids.size}`);
if (rosterPromotions.length !== people.length) errors.push(`promotion count ${rosterPromotions.length} != people ${people.length}`);
if (duplicateRosterPromotions.length !== 10) errors.push(`expected 10 duplicate promotions, got ${duplicateRosterPromotions.length}`);
...
console.log(JSON.stringify({ ok: true, people: people.length, relationships: relationships.length, placements: placements.length, groupMembers: groupMembers.length, rosterPromotions: rosterPromotions.length, duplicateRosterPromotions: duplicateRosterPromotions.length }, null, 2));
