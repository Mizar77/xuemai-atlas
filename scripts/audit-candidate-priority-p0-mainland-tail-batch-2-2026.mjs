...
const bundle = "/private/tmp/candidate-p0-mainland-tail-batch-2-2026.mjs";
execFileSync(path.join(root, "node_modules/.bin/esbuild"), [
  path.join(root, "app/candidate-priority-p0-mainland-tail-batch-2-2026.ts"),
  "--bundle", "--platform=node", "--format=esm", `--outfile=${bundle}`,
], { stdio: "ignore" });

const batch = await import(`file://${bundle}?t=${Date.now()}`);
const people = batch.candidatePriorityP0MainlandTailBatch2People2026;
const supportingPeople = batch.candidatePriorityP0MainlandTailBatch2SupportingPeople2026;
const relationships = batch.candidatePriorityP0MainlandTailBatch2Relationships2026;
const placements = batch.candidatePriorityP0MainlandTailBatch2Placements2026;
const groupMembers = batch.candidatePriorityP0MainlandTailBatch2GroupMembers2026;
const promotions = batch.candidatePriorityP0MainlandTailBatch2RosterPromotions2026;
...
const expectedExistingEndpoints = new Set(["jie-tang-thu", "bo-zhang-thu-historical"]);
