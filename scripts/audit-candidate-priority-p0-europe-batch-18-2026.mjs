...
const bundle = "/private/tmp/candidate-p0-europe-batch-18-2026.mjs";
execFileSync(path.join(root, "node_modules/.bin/esbuild"), [path.join(root, "app/candidate-priority-p0-europe-batch-18-2026.ts"), "--bundle", "--platform=node", "--format=esm", `--outfile=${bundle}`], { stdio: "ignore" });
const { people, relationships, placements, groupMembers, rosterPromotions } = await import(`file://${bundle}?t=${Date.now()}`);
const errors = []; const primary = people.filter((row) => row.primary); const ids = new Set(people.map((row) => row.id));
...
for (const person of primary) {
  ...
  else { const dimensions = execFileSync("sips", ["-g", "pixelWidth", "-g", "pixelHeight", portrait], { encoding: "utf8" }); if (!/pixelWidth:\s+512/.test(dimensions) || !/pixelHeight:\s+512/.test(dimensions)) errors.push(`${person.name}: portrait not 512 square`); }
...
for (const row of relationships) { if (!ids.has(row.from) || !ids.has(row.to)) errors.push(`${row.id}: missing endpoint`); if (!row.verified || !row.source?.url || !row.source?.supports || !row.evidenceObject) errors.push(`${row.id}: relationship evidence gate failed`); }
