...
const batchBundle = "/private/tmp/candidate-p0-asia-batch-11-audit.mjs";
const dataBundle = "/private/tmp/candidate-p0-asia-batch-11-data-audit.mjs";
execFileSync(esbuild, [path.join(root, "app/candidate-priority-p0-asia-batch-11-2026.ts"), "--bundle", "--platform=node", "--format=esm", `--outfile=${batchBundle}`], { stdio: "ignore" });
...
const endpointIds = atlasIds;

if (people.length !== 6 || batchIds.size !== 6) errors.push(`expected 6 unique people, got ${people.length}/${batchIds.size}`);
if (rosterPromotions.length !== people.length) errors.push(`promotion count ${rosterPromotions.length} != people ${people.length}`);
for (const id of batchIds) if (atlas.people.filter((person) => person.id === id).length !== 1) errors.push(`${id}: expected exactly one integrated person`);
...
for (const row of relationships) {
  ...
  if (atlas.relationships.filter((relationship) => relationship.id === row.id).length !== 1) errors.push(`${row.id}: expected exactly one integrated relationship`);
  ...
  if (row.type === "lineage" && row.subtype !== "phd_adviser") errors.push(`${row.id}: lineage direction/subtype must be adviser -> student / phd_adviser`);
  if (row.type === "collaboration" && row.subtype !== "publication") errors.push(`${row.id}: collaboration must identify an exact publication`);
}
...
const result = {
  ...
  relationshipEndpointsVerified: errors.every((item) => !item.includes("endpoint")),
  errors,
};
