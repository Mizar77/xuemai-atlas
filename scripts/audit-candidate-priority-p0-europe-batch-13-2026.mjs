...
const bundle = "/private/tmp/candidate-p0-europe-batch-13-2026.mjs";
execFileSync(path.join(root, "node_modules/.bin/esbuild"), [
  path.join(root, "app/candidate-priority-p0-europe-batch-13-2026.ts"),
  "--bundle",
  "--platform=node",
  "--format=esm",
  `--outfile=${bundle}`,
...
for (const person of people) {
  ...
  const network = relationships.filter((row) => row.from === person.id || row.to === person.id).length
