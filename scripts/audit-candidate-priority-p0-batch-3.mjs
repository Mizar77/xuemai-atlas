...
const temporaryEntry = "/private/tmp/candidate-priority-p0-batch-3-entry.ts";
fs.writeFileSync(temporaryEntry, `
  export * as asia from ${JSON.stringify(path.join(root, "app/candidate-priority-p0-asia-batch-3-2026.ts"))};
  export * as europe from ${JSON.stringify(path.join(root, "app/candidate-priority-p0-europe-batch-3-2026.ts"))};
  export * as usCanada from ${JSON.stringify(path.join(root, "app/candidate-priority-p0-us-canada-batch-3-2026.ts"))};
...
await build({
  ...
  outfile: "/private/tmp/candidate-priority-p0-batch-3.mjs",
  bundle: true,
  platform: "node",
  format: "esm",
  logLevel: "silent",
...
const current = await import(`${pathToFileURL("/private/tmp/candidate-priority-p0-batch-3.mjs").href}?v=${Date.now()}`);
...
assert(supportingPeople.length === 3, `expected three supporting relationship nodes, found ${supportingPeople.length}`);
assert(relationships.length === 12, `expected 12 verified relationships, found ${relationships.length}`);
assert(placements.length === 13, `expected 13 verified placements, found ${placements.length}`);
assert(groupMembers.length === 22, `expected 22 verified current group members, found ${groupMembers.length}`);
...
assert(queue.rosterCandidateRecords <= 6441, `candidate queue regressed above the post-batch-3 baseline: ${queue.rosterCandidateRecords}`);
assert(queue.tierCounts.P0 <= 1681, `P0 queue regressed above the post-batch-3 baseline: ${queue.tierCounts.P0}`);
assert(queue.completedBatch.promotedRosterCandidates >= 45, "queue does not record all completed candidate batches");

console.log(`Candidate P0 batch 3 audit passed: ${promotedPeople.length} promoted PIs, ${supportingPeople.length} supporting nodes, ${relationships.length} relationships, ${placements.length} placements and ${groupMembers.length} current group members; ${queue.tierCounts.P0} P0 candidates remain.`);
