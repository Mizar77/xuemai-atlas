...
const input = path.join(root, "app/candidate-priority-p0-us-canada-ready-batch-12-2026.ts");
const output = "/private/tmp/candidate-p0-usca-batch-12-audit.mjs";
...
const primary = mod.candidatePriorityP0UsCanadaReadyBatch12People2026;
const relationships = mod.relationships;
const placements = mod.placements;
const groupMembers = mod.groupMembers;
const errors = [];

if (primary.length !== 3) errors.push(`primary people ${primary.length}/3`);
...
console.log(`US/Canada P0 batch 12 audit PASS: ${primary.length} primary, ${relationships.length} relationships, ${placements.length} placements, ${groupMembers.length} group members`);
