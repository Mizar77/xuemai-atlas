...
const bundle = "/private/tmp/candidate-p0-europe-batch-12-2026.mjs";
execFileSync(path.join(root, "node_modules/.bin/esbuild"), [path.join(root, "app/candidate-priority-p0-europe-batch-12-2026.ts"), "--bundle", "--platform=node", "--format=esm", `--outfile=${bundle}`], { stdio: "ignore" });
...
for (const person of people) {
  ...
  else { const d = execFileSync("sips", ["-g", "pixelWidth", "-g", "pixelHeight", portrait], { encoding: "utf8" }); if (!/pixelWidth:\s+512/.test(d) || !/pixelHeight:\s+512/.test(d)) errors.push(`${person.name}: portrait not 512 square`); }
