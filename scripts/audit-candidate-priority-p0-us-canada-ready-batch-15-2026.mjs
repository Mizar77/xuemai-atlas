...
const input = path.join(root, "app/candidate-priority-p0-us-canada-ready-batch-15-2026.ts");
const output = "/private/tmp/candidate-p0-usca-batch-15-audit.mjs";
...
const primary = mod.candidatePriorityP0UsCanadaReadyBatch15People2026;
const errors = [];

if (primary.length !== 12) errors.push(`primary people ${primary.length}/12`);
...
for (const person of primary) {
  ...
  for (const fact of person.facts ?? []) {
    if (!fact.source?.url || !fact.source?.supports) errors.push(`${person.name}: unsourced fact ${fact.label}`);
  }
...
console.log(`US/Canada P0 batch 15 audit PASS: ${primary.length} primary, ${mod.relationships.length} relationships, ${mod.placements.length} placements, ${mod.groupMembers.length} group members, ${mod.rosterPromotions.length} promotions`);
