...
const batchBundle = "/private/tmp/candidate-p0-asia-batch-9-audit.mjs";
const dataBundle = "/private/tmp/candidate-p0-asia-batch-9-data-audit.mjs";
execFileSync(esbuild, [path.join(root, "app/candidate-priority-p0-asia-batch-9-2026.ts"), "--bundle", "--platform=node", "--format=esm", `--outfile=${batchBundle}`], { stdio: "ignore" });
...
if (people.length !== 8 || batchIds.size !== 8) errors.push(`expected 8 unique people, got ${people.length}/${batchIds.size}`);
...
for (const row of relationships) {
  ...
  if (row.subtype !== "publication") errors.push(`${row.id}: only exact publication edges are expected in Batch 9`);
}
