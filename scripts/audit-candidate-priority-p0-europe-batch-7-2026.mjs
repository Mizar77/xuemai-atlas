...
const modulePath = path.join(root, "app/candidate-priority-p0-europe-batch-7-2026.ts");
const bundlePath = "/private/tmp/candidate-p0-europe-batch-7-2026.mjs";
...
const allowedExternalEndpoints = new Set(["neil-lawrence-eu", "philip-torr-eu"]);
...
for (const edge of relationships) {
  for (const endpoint of [edge.from, edge.to]) {
    if (!ids.has(endpoint) && !allowedExternalEndpoints.has(endpoint)) {
      errors.push(`${edge.id}: unresolved endpoint ${endpoint}`);
    }
  }
...
for (const person of people) {
  ...
  if (!reviewRow || reviewRow.disposition !== "ready_batch_7") {
    errors.push(`${person.name}: batch 7 review is not ready`);
  }
}

if (review.total !== 82) errors.push(`expected 82 portrait-ready cohort records, got ${review.total}`);
if (review.byDisposition.ready_batch_7 !== 10) {
  errors.push(`expected 10 batch 7 ready records, got ${review.byDisposition.ready_batch_7}`);
}
