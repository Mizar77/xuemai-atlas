...
const modulePath = path.join(root, "app/candidate-priority-p0-europe-batch-9-2026.ts");
const bundlePath = "/private/tmp/candidate-p0-europe-batch-9-2026.mjs";
...
const review = JSON.parse(fs.readFileSync(path.join(root, "data/candidate-priority-p0-europe-batch-9-review-2026-09-03.json"), "utf8"));
...
for (const person of people) {
  ...
  for (const row of person.facts ?? []) {
    if (!row.source?.url || !row.source?.supports) errors.push(`${person.name}: fact ${row.label} lacks source URL/supports`);
  }
  if (!person.facts?.some((row) => row.label === "教育与学术训练" && row.source?.url)) errors.push(`${person.name}: missing exact sourced education fact`);
  if (!person.portrait?.source?.url || !person.portrait?.source?.supports) errors.push(`${person.name}: portrait source incomplete`);
  ...
  const reviewRow = review.records.find((row) => normalizeName(row.name) === normalizeName(person.name));
  if (!reviewRow || reviewRow.disposition !== "ready_batch_9") errors.push(`${person.name}: batch 9 review is not ready`);
}

for (const edge of relationships) {
  for (const endpoint of [edge.from, edge.to]) if (!ids.has(endpoint)) errors.push(`${edge.id}: unresolved endpoint ${endpoint}`);
  if (!edge.verified || !edge.source?.url || !edge.evidence) errors.push(`${edge.id}: relationship evidence incomplete`);
}
...
if (review.total !== 82) errors.push(`expected 82 cohort records, got ${review.total}`);
if (review.byDisposition.ready_batch_9 !== 8) errors.push(`expected 8 batch 9 ready, got ${review.byDisposition.ready_batch_9}`);
if (review.byDisposition.blocked_batch_9_specific_evidence_gap !== 18) errors.push(`expected 18 precise batch 9 blockers, got ${review.byDisposition.blocked_batch_9_specific_evidence_gap}`);
for (const row of review.records.filter((row) => row.disposition === "blocked_batch_9_specific_evidence_gap")) {
  if (!row.blocker || !(row.missingEvidence?.length > 0)) errors.push(`${row.name}: blocker is not explicit`);
}
